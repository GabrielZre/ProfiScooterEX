...
constructor(
    ...
    private val locationObserver =
        Observer<LocationDetails> { tripData ->
            tripData.let {
                ...
                when (result) {
                    is Resource.Success -> {
                        ...
                        if (startBatteryVoltage != null) {
                            ...
                            batteryPercentage =
                                calculateBatteryPercent(result.data.whEnergyConsumed)
                            if (distanceTrip >= 0.1) {
                                remainingDistance =
                                    calculateRemainingDistance(whEnergyConsumed, distanceTrip)
                                efficiencyFactor =
    ...
    private fun calculateStartBatteryPercent(): Float {
        return if (startBatteryVoltage != null) {
            val batteryPercent =
                ((startBatteryVoltage!! - scooterData.bottomCutOff.toFloat()) * 100) /
                    (scooterData.upperCutOff.toFloat() - scooterData.bottomCutOff.toFloat())
    ...
    private fun calculateTheoreticalRange(): Float {
        val energyCapacityWh =
            scooterData.batteryAh.toFloat() * scooterData.batteryVoltage.toFloat()

        val rangeKm = (energyCapacityWh) / (scooterData.motorWatt.toFloat() / 25)
        return rangeKm
    }
    private fun calculateMaxDistance(): Float {
        val constantBatteryWh =
            scooterData.batteryAh.toFloat() * scooterData.batteryVoltage.toFloat()

        return (constantBatteryWh) / whEnergyConsumed * distanceTrip
    }

    private fun calculateEfficiencyFactor(
        consumptionBasedMaxRange: Float,
        theoreticalMaxRange: Float
    ): Float {
        val efficiencyFactor =
            if (consumptionBasedMaxRange <= theoreticalMaxRange) {
